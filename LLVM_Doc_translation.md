...
## リンケージ型
すべてのグローバル変数と関数には、次のタイプのリンケージがあります:
- private
    - イベート」リンケージを持つグローバル値は、現在のモジュール内のオブジェクトによって直接アクセス可能です。 特に、プライベートグローバル値を持つモジュールにコードをリンクすると、衝突を避けるために必要に応じてプライベートの名前を変更する可能性があります。 シンボルはモジュール専用であるため、すべての参照を更新できます。 これは、オブジェクトファイル内のシンボルテーブルには表示されません。
...
##  Structure Types
LLVM IRでは、「特定」と「リテラル」の両方の構造タイプを指定できます。 リテラル型は構造的にユニークですが、識別される型は一意ではありません。 不透明な構造型を使用して、まだ使用できない型を宣言することもできます。

識別された構造体指定の例は次のとおりです。
``` %mytype = type { %mytype*, i32 } ```

LLVM 3.0リリース以前は、識別されたタイプは構造的にユニークでした。 LLVMの最新バージョンでは、リテラルタイプのみがユニークです。

## 非整数ポインタ型
注：非整数型のポインタ型は進行中の作業であり、現時点では実験的に考慮する必要があります。

LLVM IRはオプションで、フロントエンドが特定のアドレス空間のポインタをdatalayout文字列を介して「非整数」として示すことができます。 非整数型のポインタ型は、指定されていないビット単位の表現を持つポインタを表します。 すなわち、積分表現は、ターゲットに依存するかまたは不安定である（固定された整数に裏打ちされていない）。

整数を非整数ポインタ型に変換するinttoptr命令は型が悪いので、非整数型のポインタ型の値を整数に変換する命令もあります。 前記命令のベクトルバージョンも不正なタイプである。
